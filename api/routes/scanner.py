...
def get_scanner_status(db: Session = Depends(get_db)):
    ...
    return {"scanner_running": True, "total_enabled_stocks": total_stocks, "signals_last_hour": recent_signals, "errors_last_hour": recent_errors, "last_scan_time": None}


@router.post("/start")
def start_scanning(db: Session = Depends(get_db)):
    """Start scanner service"""
    return {"message": "Scanner service started", "scanner_running": True, "timestamp": datetime.utcnow().isoformat()}


@router.post("/stop")
def stop_scanning(db: Session = Depends(get_db)):
    """Stop scanner service"""
    return {"message": "Scanner service stopped", "scanner_running": False, "timestamp": datetime.utcnow().isoformat()}
...
def scan_single_stock(symbol: str, db: Session = Depends(get_db)):
    ...
    log_entry = SystemLog(level="INFO", component="scanner_api", message=f"Manual scan triggered for {symbol}", details={"triggered_by": "api"})
    db.add(log_entry)
    db.commit()
...
def get_scanner_config(db: Session = Depends(get_db)):
    ...
    return {"scan_interval": configs.get("scan_interval", "30"), "high_priority_interval": configs.get("high_priority_interval", "10"), "low_priority_interval": configs.get("low_priority_interval", "60"), "max_workers": configs.get("max_workers", "10")}
...
def get_scanner_logs(level: str = Query(None), component: str = Query(None), limit: int = Query(100, ge=1, le=1000), db: Session = Depends(get_db)):
    ...
    return [{"id": log.id, "timestamp": log.timestamp.isoformat() if log.timestamp else None, "level": log.level, "component": log.component, "message": log.message, "details": log.details} for log in logs]
